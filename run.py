...
def configure_environment_sequentially(dev_mode=False, force_reconfigure=False, easy_mode=True, reconfigure_inference=False):
    ...
    if easy_mode:
        ...
        if is_placeholder(current_jwt):
            print(f"🔄 JWT_SECRET has placeholder value '{current_jwt}'.")
        print(
            f"{C_CYAN}ℹ️  JWT_SECRET is optional. If left blank, the backend will "
            f"auto-generate one on first start. You can change it later from the "
            f"TT Studio Settings UI.{C_RESET}"
        )
