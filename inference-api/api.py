...
async def run_inference(request: RunRequest):
    ...
    try:
        ...
        # If impl is not provided, try to get default from MODEL_SPECS
        if request.impl:
            sys.argv.extend(["--impl", request.impl])
        else:
            # Try to find default implementation from MODEL_SPECS
            try:
                from workflows.model_spec import get_runtime_model_spec
                model_spec = get_runtime_model_spec(request.model, request.device)
                if model_spec and hasattr(model_spec, 'impl') and model_spec.impl:
                    sys.argv.extend(["--impl", model_spec.impl])
                    logger.info(f"Using default implementation '{model_spec.impl}' for model '{request.model}'")
            except Exception as e:
                logger.warning(f"Could not determine default implementation for model '{request.model}': {e}")
                # If we can't determine it, let run.py handle the error with a clearer message
        ...
        try:
            ...
            if return_code == 0:
                ...
                # Handle case where container_info might be None
                if container_info and isinstance(container_info, dict):
                    container_name = container_info.get("container_name")
                    container_id = container_info.get("container_id")
                    docker_log_file_path = container_info.get("docker_log_file_path")
                else:
                    # If container_info is None or not a dict, set defaults
                    container_name = None
                    container_id = None
                    docker_log_file_path = None
                    logger.warning("container_info is None or not a dict, using defaults")
