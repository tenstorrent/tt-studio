...
export function useBugReport() {
  ...
  const createIssue = useCallback(async () => {
    ...
    setIssueResult(null);
    try {
      // The backend builds the Jira summary/description (and the GitHub
      // fallback URL when Jira isn't configured) from these structured fields,
      // classifies the failing component, and attaches the log ZIP server-side.
      const response = await fetch("/logs-api/jira-issue/", {
        method: "POST",
      ...
      if (result.created_via_api) {
        ...
        // No Jira credentials — open the pre-filled GitHub issue URL instead.
