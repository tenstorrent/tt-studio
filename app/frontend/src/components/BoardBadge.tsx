...
const BoardBadge: React.FC<BoardBadgeProps> = ({ boardName, className = "", onClick }) => {
  ...
  if (onClick) {
    return (
      <button onClick={onClick} className="border-none bg-transparent p-0 focus:outline-none focus:ring-2 focus:ring-TT-purple-accent focus:ring-opacity-50 rounded-full">
        {BadgeContent}
      </button>
    );
  }
