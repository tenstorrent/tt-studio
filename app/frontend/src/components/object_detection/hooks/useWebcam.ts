  ...
  const handleStartCapture = useCallback(async () => {
    ...
    try {
      ...
      const sendSnapshotInterval = () => {
        sendSnapshot(videoRef, setDetections);
      };

      sendSnapshotInterval();
      intervalRef.current = window.setInterval(sendSnapshotInterval, 1000);
    } catch (error) {
      console.error("Error accessing webcam:", error);
      setIsLoading(false);
    }
  }, [setDetections, setLiveMode, setIsLoading, setIsStreaming, setIsCameraOn]);
