...
const Stepper = React.forwardRef<HTMLDivElement, StepperProps>(
  (props, ref: React.Ref<HTMLDivElement>) => {
    ...
    const items: React.ReactElement<StepElementProps>[] = [];

    const footer = childArr.map((child, _index) => {
      if (!React.isValidElement<StepElementProps>(child)) {
  ...
  return (
    <>
      {React.Children.map(children, (child, i) => {
        const isCompletedStep = React.isValidElement<StepElementProps>(child)
...
const HorizontalContent = ({ children }: { children: React.ReactNode }) => {
  ...
  const currentStepNode = childArr[activeStep];

  if (!React.isValidElement<StepElementProps>(currentStepNode)) {
    return null;
  }

  return <>{currentStepNode.props.children}</>;
};
    ...
    return (
      <div
        ...
        className={cn(
          ...
          "data-[completed=true]:[&:not(:last-child)]:after:bg-TT-purple-accent",
      ...
      >
        <div
          ...
          className={cn(
            ...
            "flex items-center",
            variant === "line" &&
              "border-s-[3px] data-[active=true]:border-cTT-purple-accent py-2 ps-3",
    ...
    return (
      <div
        ...
        className={cn(
          ...
          variant === "line" &&
            "flex-col flex-1 border-t-[3px] data-[active=true]:border-TT-purple-accent",
  ...
  return (
    <Button
      ...
      className={cn(
        ...
        "data-[current=true]:border-TT-purple-accent data-[current=true]:bg-secondary",
