...
def run_container(impl):
    ...
    try:
        ...
        run_kwargs.update({"network": NETWORK_NAME})
        ...
        verify_container(impl, container)
        return {
            "status": "success",
            "container_id": container.id,
            "container_name": container.name,
            "service_route": impl.service_route,
            "port_bindings": impl.docker_config["ports"],
        }
...
def verify_container(impl, container):
    verify_port_mappings(impl, container)


def verify_port_mappings(impl, container):
    impl_port_bindings = impl.docker_config["ports"]
